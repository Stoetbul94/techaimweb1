  ...
  return (
    <div className="border border-brand-border bg-brand-panel">
      <div className="flex items-center gap-2 border-b border-brand-border bg-brand-surface px-4 py-3">
        <div className="h-2.5 w-2.5 rounded-full bg-brand-cta" />
        <div className="h-2.5 w-2.5 rounded-full bg-brand-warning" />
        <div className="h-2.5 w-2.5 rounded-full bg-brand-success" />
        <span className="ml-2 font-mono text-xs text-brand-text">TECH AIM ARMS — {feature.label}</span>
      </div>
      <div className="grid gap-4 p-6 md:grid-cols-3">
        <div className="md:col-span-2">
          <h4 className="font-heading text-xl font-bold text-white">{feature.title}</h4>
          ...
          <div className="mt-6 h-48 border border-brand-border bg-brand-bg">
            <svg viewBox="0 0 400 180" className="h-full w-full" aria-hidden="true">
              <polyline fill="none" stroke="#3B9EFF" strokeWidth="2" points="10,150 50,120 90,130 130,90 170,100 210,70 250,80 290,50 330,60 370,40" />
            </svg>
          </div>
        </div>
        <div className="space-y-3">
          {["Lane 1: Active", "Lane 2: Active", "Lane 3: Idle"].map((lane) => (
            <div key={lane} className="border border-brand-border p-3 font-mono text-xs">
              <span className="text-brand-accent">●</span> {lane}
            </div>
          ))}
          <div className="border border-brand-accent/30 bg-brand-accent/5 p-3">
            <p className="font-mono text-xs text-brand-accent">SESSION STATS</p>
            <p className="mt-2 font-heading text-2xl font-bold text-white">10.28</p>
