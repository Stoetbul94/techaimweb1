...
const BulletHeroScene = dynamic(() => import("@/components/three/BulletHeroScene"), {
  ...
  loading: () => (
    <div className="grid min-h-[430px] place-items-center border border-brand-border bg-brand-panel/50">
      <div className="h-12 w-12 animate-spin rounded-full border-2 border-brand-border border-t-brand-accent" />
    </div>
  ),
});
...
export default function Hero() {
  return (
    <section className="relative min-h-screen overflow-hidden bg-brand-bg">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(59,158,255,0.08),transparent_34%)]" />
      <motion.div
      ...
      <div className="relative mx-auto grid min-h-screen max-w-7xl items-center gap-12 px-4 pt-28 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
        <motion.div initial="hidden" animate="visible" transition={{ staggerChildren: 0.15 }} className="relative z-10 max-w-3xl">
          <motion.p variants={fadeUp} className="font-body text-[11px] uppercase tracking-[0.2em] text-brand-accent">
            Precision measurement platform
          </motion.p>
          <motion.h1
            variants={fadeUp}
            className="mt-6 font-heading text-[clamp(2.75rem,7vw,5.5rem)] font-bold leading-[0.98] text-white"
          >
            Precision Electronic Target Systems
          </motion.h1>
          <motion.p variants={fadeUp} className="mt-8 max-w-xl text-lg leading-8 text-brand-text">
          ...
          <motion.p variants={fadeUp} className="mt-8 font-mono text-xs text-brand-text/80">
      ...
      <motion.div
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 text-brand-accent"
      >
        <ChevronDown size={30} />
      </motion.div>
    </section>
