  ...
  return (
    <section className="border-t border-brand-border bg-brand-elevated py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="font-body text-xs uppercase tracking-[0.2em] text-brand-crimson">How it works</p>
        ...
        >
          {steps.map((step, i) => (
            <motion.div key={step.label} variants={fadeUp} className="relative flex flex-1 flex-col items-center lg:px-4">
              {i < steps.length - 1 && (
                <motion.div className="absolute left-1/2 top-12 hidden h-px w-full bg-gradient-to-r from-brand-crimson/50 to-transparent lg:block" />
              )}
              <div className="relative z-10 grid h-16 w-16 place-items-center border border-brand-crimson bg-brand-panel">
                <step.icon className="h-7 w-7 text-brand-crimson" />
              </div>
              {i < steps.length - 1 && (
                <div className="my-2 h-8 w-px bg-brand-border lg:hidden" aria-hidden="true" />
              )}
              <h3 className="mt-6 text-center font-heading text-lg font-semibold text-brand-text-primary">{step.label}</h3>
