    ...
    <main className="bg-brand-bg pt-20">
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          <p className="font-body text-xs uppercase tracking-[0.22em] text-brand-crimson">Telemetry core</p>
          <h1 className="mt-5 max-w-4xl font-heading text-5xl font-bold text-brand-text-primary md:text-7xl">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-3">
          {pillars.map(([title, Icon, body, specs]) => {
            ...
            return (
              ...
              >
                <IconComponent className="text-brand-crimson" size={32} />
                <h2 className="mt-5 font-heading text-2xl font-bold text-brand-text-primary">{title as string}</h2>
                <p className="mt-4 leading-7">{body as string}</p>
                <ul className="mt-5 space-y-2 text-sm text-brand-crimson">
        ...
        <motion.section initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-16 overflow-x-auto">
          <h2 className="font-heading text-4xl font-bold text-brand-text-primary">TechAim vs Traditional Scoring</h2>
          <table className="mt-8 w-full min-w-[760px] border-collapse border border-brand-border text-left">
            <thead className="bg-brand-panel text-brand-text-primary">
              <tr>
            ...
            <tbody>
              {["Accuracy", "Response Time", "Data Analytics", "Remote Monitoring", "Upgrade Path", "Support"].map((feature) => (
                <tr key={feature}>
                  <td className="border border-brand-border p-4 text-brand-text-primary">{feature}</td>
                  <td className="border border-brand-border p-4">Limited</td>
                  <td className="border border-brand-border p-4">Available</td>
                  <td className="border border-brand-border p-4 text-brand-crimson">Optimized</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <section className="mt-16 py-14 text-center">
          <p className="mx-auto max-w-4xl font-heading text-3xl leading-tight text-brand-text-primary">
