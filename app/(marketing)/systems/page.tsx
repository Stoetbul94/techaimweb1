    ...
    <main className="bg-brand-bg pt-20">
      <section className="grid min-h-[50vh] place-items-center border-b border-brand-border bg-[radial-gradient(circle_at_80%_20%,rgba(0,200,255,0.09),transparent_32%)] px-4">
        <div className="mx-auto max-w-4xl text-center">
          <p className="font-body text-xs uppercase tracking-[0.22em] text-brand-cyan">Product range</p>
          <h1 className="mt-5 font-heading text-5xl font-bold text-white md:text-7xl">
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <h2 className="font-heading text-4xl font-bold text-white">Available Now</h2>
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {currentProducts.map((product) => (
            <article key={product.name} className="border-t border-brand-cyan bg-brand-panel p-6">
              <div className="aspect-video border border-dashed border-brand-cyan bg-[#0A0D14] grid place-items-center text-sm" aria-label={`${product.name} image placeholder`}>
                Product Image Coming Soon
              </div>
              <span className="mt-6 inline-block bg-brand-red px-3 py-1 text-xs font-medium text-white">
                {product.tag}
              </span>
              <h3 className="mt-4 font-heading text-3xl font-bold text-white">{product.name}</h3>
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <Link href="/contact" className="border border-brand-cyan px-5 py-3 text-center font-heading text-sm font-semibold text-brand-cyan">
                  View Specs
                </Link>
                <Link href="/contact" className="bg-brand-red px-5 py-3 text-center font-heading text-sm font-semibold text-white">
                  Request Quote
                </Link>
              </div>
            </article>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4">
          <h2 className="font-heading text-4xl font-bold text-white">Coming Soon</h2>
          <span className="bg-brand-red px-3 py-1 text-xs font-semibold text-white">In Development</span>
        </div>
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {futureProducts.map(([name, discipline]) => (
            <article key={name} className="border border-brand-border bg-brand-panel p-6 opacity-70">
              ...
              <Link href="/contact" className="mt-6 inline-block border border-brand-cyan px-4 py-2 text-sm text-brand-cyan">
                Notify Me
              </Link>
            </article>
          ))}
