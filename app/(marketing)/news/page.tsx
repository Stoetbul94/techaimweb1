  ...
  return (
    <main className="bg-brand-bg pt-20">
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <p className="font-body text-xs uppercase tracking-[0.22em] text-brand-cyan">Resources</p>
        <h1 className="mt-5 font-heading text-5xl font-bold text-white md:text-7xl">News & ISSF</h1>
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {posts.map((post) => (
            <article key={post.title} className="border border-brand-border bg-brand-panel p-6">
              <p className="text-sm text-brand-cyan">{post.date}</p>
