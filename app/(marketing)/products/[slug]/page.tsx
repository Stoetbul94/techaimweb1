...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="bg-brand-bg pt-20">
      <section className="border-b border-brand-border bg-[radial-gradient(circle_at_80%_20%,rgba(59,158,255,0.08),transparent_32%)]">
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-6 font-heading text-4xl font-bold text-white md:text-6xl">{product.name}</h1>
          <p className="mt-4 max-w-2xl text-xl text-brand-text-bright">{product.headline}</p>
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[200px_1fr] lg:px-8">
        ...
        <div className="space-y-20">
          ...
          <section id="features">
            <h2 className="mb-6 font-heading text-2xl font-bold text-white">Features</h2>
            <FeatureGrid features={product.features} />
          </section>
          <section id="drawings">
            <h2 className="mb-6 font-heading text-2xl font-bold text-white">Technical Drawings</h2>
            <TechnicalDrawing />
          </section>
          <section id="compatibility">
            <h2 className="mb-6 font-heading text-2xl font-bold text-white">Compatibility</h2>
            <CompatibilityMatrix items={product.compatibility} />
          </section>
          <section id="installation">
            <h2 className="font-heading text-2xl font-bold text-white">Installation</h2>
            <ul className="mt-4 space-y-3">
              {product.installation.map((item) => (
                <li key={item} className="flex items-start gap-3 text-sm leading-7">
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 bg-brand-accent" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <section id="faq">
            <h2 className="mb-6 font-heading text-2xl font-bold text-white">FAQ</h2>
          ...
          {related.length > 0 && (
            <section>
              <h2 className="mb-6 font-heading text-2xl font-bold text-white">Related Products</h2>
