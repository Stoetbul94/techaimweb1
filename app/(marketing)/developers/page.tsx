    ...
    <main className="bg-brand-bg">
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="border border-brand-border bg-brand-panel p-6">
          <h2 className="font-heading text-xl font-bold text-white">Authentication</h2>
          ...
          <pre className="mt-4 overflow-x-auto border border-brand-border bg-brand-bg p-4 font-mono text-xs text-brand-accent">
        ...
        {tags.map((tag) => (
          <div key={tag} className="mt-12">
            <h2 className="font-heading text-2xl font-bold text-white">{tag}</h2>
                ...
                .map((ep) => (
                  <article key={`${ep.method}${ep.path}`} className="border border-brand-border bg-brand-panel p-6">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="bg-brand-accent px-2 py-1 font-mono text-xs font-bold text-[#050505]">{ep.method}</span>
                      <code className="font-mono text-sm text-white">{ep.path}</code>
                    </div>
                    <p className="mt-3 text-sm">{ep.summary}</p>
                    <pre className="mt-4 overflow-x-auto border border-brand-border bg-brand-bg p-4 font-mono text-xs text-brand-text-bright">
        ...
        <div className="mt-12">
          <h2 className="font-heading text-2xl font-bold text-white">WebSocket — Live Scoring</h2>
          <p className="mt-3 text-sm">{wsEndpoint.description}</p>
          <code className="mt-2 block font-mono text-brand-accent">{wsEndpoint.path}</code>
          <pre className="mt-4 border border-brand-border bg-brand-bg p-4 font-mono text-xs">
            {JSON.stringify(wsEndpoint.exampleMessage, null, 2)}
          </pre>
        </div>

        <div className="mt-12">
          <h2 className="font-heading text-2xl font-bold text-white">Webhook Events</h2>
          <div className="mt-6 divide-y divide-brand-border border border-brand-border">
            {webhookEvents.map((w) => (
              <div key={w.event} className="grid gap-2 px-4 py-3 sm:grid-cols-[200px_1fr]">
                <code className="font-mono text-sm text-brand-accent">{w.event}</code>
